...
def answer(rag_url, key, store, questionnaire, history_table, sub, question,
           at=None, app=False, recommendation=False, timeout=chat.TIMEOUT_SECONDS,
           context=None) -> dict:
    ...
    `app` marks the chat as one the app composed. `recommendation` marks it as the user's one
    next-meal recommendation chat. The grounding block sent beside the question is the asker's
    recent-data block unless the caller hands over `context` of its own, as the weekly recap
