...
the answering LLM reads domain terms rather than internal ids. The question text stays first:
the upstream service embeds the whole string for retrieval, and the head is what should drive
the match."""
...
def with_user_context(question, store, questionnaire, sub, day) -> str:
    ...
    upstream bare. Absent data is a legal domain state and still rides (empty summaries let
    the LLM say nothing was tracked); false meal flags and empty addition lists are omitted
    from the block as the equally legal quiet state."""
    data = {
...
def _grade_label(labels, choice, small_portion) -> str:
    if small_portion:
        return f"{labels[choice]} (כמות קטנה)"
    return labels[choice]


def _meal_entry(meal, grade_labels, addition_labels) -> dict:
    entry = {"שעה": meal["at"][11:16],
             "מקור פחמימה": _grade_label(grade_labels, meal["carbs_choice"],
                                         meal["small_portion"])}
    second = meal["second_source"]
    if second is not None:
        entry["מקור פחמימה נוסף"] = _grade_label(grade_labels, second["carbs_choice"],
    ...
    if meal["fruit"]:
        entry["פרי"] = True
    if meal["additions"]:
        entry["תוספות"] = [addition_labels[addition] for addition in meal["additions"]]
    return entry
