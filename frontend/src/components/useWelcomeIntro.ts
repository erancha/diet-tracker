...
// The banner's four sentences in turn, the target-weight line, a quiet rest, and then the
// standing add-meal invitation; "meal" is terminal.
export type IntroStage = 0 | 1 | 2 | 3 | 4 | "rest" | "meal";
...
// The mail sentence holds longest of the four: it is the longest to read and ends in a button
// the reader may want to press before the banner folds.
export const INTRO_STAGE_MS: Record<0 | 1 | 2 | 3 | 4, number> = {
  0: 3_000, 1: 2_000, 2: 2_000, 3: 5_000, 4: TARGET_FLASH_MS,
};
...
export function useWelcomeIntro(active: boolean): IntroStage | null {
  ...
  useEffect(() => {
    ...
    const [next, holdMs]: [IntroStage, number] = stage === "rest"
      ...
      : [stage === 4 ? "rest" : ((stage + 1) as IntroStage), INTRO_STAGE_MS[stage]];
