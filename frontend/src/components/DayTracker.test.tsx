...
describe("DayTracker", () => {
  ...
  it("records one fat serving on a tick, more from the count beside it, and clears after saving", () => {
    ...
    expect(screen.queryByLabelText("מנות — מנת שומן")).toBeNull();
    expect(screen.queryByText("מנת שומן = כף שמן")).toBeNull();
    fireEvent.click(screen.getByLabelText("מנת שומן"));
    const hint = screen.getByText("מנת שומן = כף שמן");
    expect(hint.previousElementSibling).toBe(screen.getByLabelText("מנת שומן").closest("label"));
    expect(screen.getByLabelText("מנות — מנת שומן")).toHaveValue("1");
    ...
    expect(screen.queryByText("מנת שומן = כף שמן")).toBeNull();
    expect(screen.getByLabelText("מנות — מנת שומן")).toHaveValue("1");
    fireEvent.change(screen.getByLabelText("מנות — מנת שומן"), { target: { value: "3" } });
    fireEvent.click(screen.getByRole("button", { name: "שמירת ארוחה" }));
    expect(onAddMeal).toHaveBeenCalledWith(expect.objectContaining({ fat_servings: 3 }));
    openMealForm();
    expect(screen.getByLabelText("מנת שומן")).not.toBeChecked();
  });
  ...
  it("opens a recorded meal's servings for correction and unticking drops them", () => {
    ...
    expect(screen.getByLabelText("מנת שומן")).toBeChecked();
    expect(screen.getByLabelText("מנות — מנת שומן")).toHaveValue("2");
    fireEvent.click(screen.getByLabelText("מנת שומן"));
    fireEvent.click(screen.getByRole("button", { name: "שמירת ארוחה" }));
  ...
  it("opens an edited addition on the amount it was recorded at", () => {
    ...
    render(<DayTracker expandLabels={false} suggestBeforeHours={1} treatDay={TREAT_DAY} maxMealsPerDay={NO_CAP_MEALS} closeMinWindowHours={6} closeFrom={CLOSE_FROM} stretchesUntil={STRETCHES_UNTIL} questionnaire={questionnaire} day={day}
                       ...
                       onDeleteMeal={vi.fn()} onCloseDay={vi.fn()} />);
    fireEvent.click(screen.getByRole("button", { name: "עריכת ארוחה 09:10" }));
    expect(screen.getByLabelText("מתוק")).toBeChecked();
    expect(screen.getByLabelText("כמות — מתוק")).toHaveValue("much");
  });
  ...
  it("reads the grades at the density the menu sets, in the picker and the rows alike", () => {
    ...
    const tracker = (expandLabels: boolean) =>
      <DayTracker expandLabels={expandLabels} suggestBeforeHours={1} treatDay={TREAT_DAY} maxMealsPerDay={NO_CAP_MEALS} closeMinWindowHours={6} closeFrom={CLOSE_FROM} stretchesUntil={STRETCHES_UNTIL} questionnaire={questionnaire} day={trackedDay}
                  ...
                  onAddMeal={vi.fn()} onUpdateMeal={vi.fn()}
                  onDeleteMeal={vi.fn()} onCloseDay={vi.fn()} />;
    const { rerender } = render(tracker(false));
    openMealForm();
    ...
    rerender(tracker(true));
    // One setting reaches the grade group and the recorded meal row alike.
  ...
  it("loads a recorded meal into the meal form when its edit button is tapped", () => {
    ...
    expect(screen.getByLabelText("פרי")).toBeChecked();
    expect(screen.getByLabelText("ירקות")).not.toBeChecked();
  ...
  it("keeps a diverged edit when its discard dialog is dismissed", () => {
    ...
    expect(screen.getByRole("button", { name: "עדכון ארוחה" })).toBeInTheDocument();
    expect(screen.getByLabelText("ירקות")).toBeChecked();
    expect(screen.getByLabelText("שעת הארוחה")).toHaveValue("13:30");
  });
  ...
  it("disables a meal's delete button only while its deletion is in flight", () => {
    const props = { maxMealsPerDay: NO_CAP_MEALS, closeMinWindowHours: 6, closeFrom: CLOSE_FROM, stretchesUntil: STRETCHES_UNTIL, questionnaire, day: trackedDay,
                    ...
                    onAddMeal: vi.fn(), onUpdateMeal: vi.fn(), onDeleteMeal: vi.fn(),
                    onCloseDay: vi.fn() };
    const { rerender } = render(<DayTracker expandLabels={false} suggestBeforeHours={1} treatDay={TREAT_DAY} {...props} deletingMealId="b" />);
    ...
    rerender(<DayTracker expandLabels={false} suggestBeforeHours={1} treatDay={TREAT_DAY} {...props} deletingMealId={undefined} />);
  ...
  it("the close flow does not linger past the close into a reopened day", () => {
    ...
    const { rerender } = render(<DayTracker expandLabels={false} suggestBeforeHours={1} treatDay={TREAT_DAY} {...props} />);
    fireEvent.click(screen.getByRole("button", { name: "סגירת יום" }));
    fireEvent.click(screen.getByLabelText("3 ליטר"));
    fireEvent.click(screen.getByRole("button", { name: "אישור וסגירה" }));
    ...
    rerender(<DayTracker expandLabels={false} suggestBeforeHours={1} treatDay={TREAT_DAY} {...props} closed />);
    rerender(<DayTracker expandLabels={false} suggestBeforeHours={1} treatDay={TREAT_DAY} {...props} closed={false} />);
...
describe("DayTracker score breakdown", () => {
  ...
  it("swaps the meal list and form for the score's breakdown from the heavy score, and back from it", () => {
    ...
    expect(screen.getByText("מתוק · הרבה")).toBeInTheDocument();
