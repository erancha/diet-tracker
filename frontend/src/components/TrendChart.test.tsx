...
describe("TrendChart", () => {
  it("renders one titled panel per chartable question plus the violation legend", () => {
    render(<TrendChart questionnaire={fixtureQuestionnaire} days={days} today={emptyToday} endDate="2026-08-18" loadedInMs={0} />);
  ...
  it("wraps each panel in a trend-panel container so panels are visually separated", () => {
    const { container } = render(<TrendChart questionnaire={fixtureQuestionnaire} days={days} today={emptyToday} endDate="2026-08-18" loadedInMs={0} />);
  ...
  it("labels the chart with how long its data took to reach the browser", () => {
    render(<TrendChart questionnaire={fixtureQuestionnaire} days={days} today={emptyToday} endDate="2026-08-18" loadedInMs={150} />);
  ...
  it("shows no timing label to a viewer the load diagnostics are not meant for", () => {
    render(<TrendChart questionnaire={fixtureQuestionnaire} days={days} today={emptyToday} endDate="2026-08-18" loadedInMs={null} />);
    expect(screen.queryByText(/טעינה/)).not.toBeInTheDocument();
  });

  it("renders the panels before any day has been submitted", () => {
    const { container } = render(<TrendChart questionnaire={fixtureQuestionnaire} days={[]} today={emptyToday} endDate="2026-08-19" loadedInMs={0} />);
    expect(container.querySelectorAll(".trend-panel")).toHaveLength(2);
  });

  it("charts the carb-score panel first even when it is configured last", () => {
    const { container } = render(<TrendChart questionnaire={withCarbsPanel} days={days} today={emptyToday} endDate="2026-08-18" loadedInMs={0} />);
