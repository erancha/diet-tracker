...
deploy_cognito() {
  ...
  aws cloudformation deploy --template-file build/template-cognito.pkg.yaml \
    ...
    --parameter-overrides GoogleClientId="$GOOGLE_CLIENT_ID" GoogleClientSecret="$GOOGLE_CLIENT_SECRET" \
      AllowedEmails="$ALLOWED_EMAILS" AdminEmail="$ADMIN_EMAIL" SesSender="$SES_SENDER" \
...
# browser as availability alone — the URL and its key stay on the backend.
if [ -n "$RAG_API_URL" ]; then CHAT_AVAILABLE=true; else CHAT_AVAILABLE=false; fi
