...
FAT_HEADING = questionnaire.question("fat").day_heading
...
def test_a_day_with_no_meals_asks_for_the_first_meal_with_zero_facts():
    ...
    assert f"{FAT_HEADING} היום: 0" in question
...
def test_the_facts_count_todays_meals_vegetables_fat_and_fruit():
    meals = [
        meal("2026-09-24T08:00:00+03:00", "carb_grade_1", vegetables=True, fat_servings=1),
        meal("2026-09-24T13:00:00+03:00", "carb_grade_2", fruit=True, fat_servings=2),
    ...
    assert f"{FAT_HEADING} היום: 3" in question
...
def test_the_only_hard_coded_words_are_the_request_and_the_fact_names():
    ...
    # The fat figure is named by its configured heading, never by its id.
