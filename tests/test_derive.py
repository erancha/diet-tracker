...
def test_derivation_vectors(vector):
    result = derive(vector["meals"], FIXTURE["weights"], FIXTURE["addition_values"])
...
def test_unknown_carbs_choice_raises():
    ...
    with pytest.raises(KeyError):
        derive([meal], FIXTURE["weights"], FIXTURE["addition_values"])


def test_unknown_addition_raises():
    meal = {"at": "2026-08-20T09:00:00+03:00", "carbs_choice": "grade1", "vegetables": False,
            "fruit": False, "additions": ["nope"]}
    with pytest.raises(KeyError):
        derive([meal], FIXTURE["weights"], FIXTURE["addition_values"])
